  ...
  useEffect(() => {
    ...
    async function fetchTenantData() {
      const [paymentsRes, leaseRes] = await Promise.all([
        ...
        supabase
          .from('leases')
          .select('id, rent_amount, end_date, security_deposit, status')
          .eq('tenant_id', selected!.id)
          .eq('status', 'active')
          .maybeSingle(),
      ])
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 space-y-3">
              <div className="flex items-center justify-between mb-3 px-1">
                <span className="text-xs font-bold uppercase tracking-widest text-outline">All Tenants</span>
                <span className="text-xs font-bold text-primary">{filtered.length} shown</span>
              </div>
              {filtered.map(tenant => (
                <button
                  key={tenant.id}
                  onClick={() => { setSelected(tenant); setActiveTab('payments') }}
                  className={cn(
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      tenant.status === 'active' ? 'bg-secondary-fixed text-primary' : 'bg-surface-container-high text-on-surface-variant'
                    )}>
